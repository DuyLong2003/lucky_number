...
import { getParticipants, drawWinner, resetDraw, Participant, isAuthenticated, setOnUnauthorizedCallback } from '../services/api';
...
export const LuckyDrawView: React.FC = () => {
  ...
  const [loginTriggeredBy, setLoginTriggeredBy] = useState<'init' | 'draw' | 'history' | 'reset' | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // If 401, login modal will be shown by the callback
  ...
  return (
    <>
      ...
      <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
      {/* Main Draw Area */}
      <div className="md:col-span-2 bg-red-800 border-4 border-yellow-500 rounded-3xl p-8 shadow-2xl relative">
        <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-yellow-500 text-red-900 px-6 py-1 rounded-full font-bold shadow-lg uppercase text-sm">
          Bảng Quay Số
        </div>
        ...
        <div className="text-center py-12">
          ...
          <div className="flex flex-col items-center gap-4">
            <button
              ...
              className={`group relative px-12 py-4 bg-yellow-500 rounded-2xl font-black text-red-900 text-xl uppercase tracking-tighter shadow-[0_8px_0_rgb(180,83,9)] active:shadow-none active:translate-y-2 transition-all disabled:opacity-50 disabled:translate-y-2 disabled:shadow-none`}
            >
            ...
            (() => {
              return winners.map((winner, idx) => {
                ...
                return (
                  <div 
                    key={winner.id} 
                    className={`flex items-center justify-between p-3 rounded-xl animate-slideInRight transition-all ${
                      rankInfo 
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className={`font-mono text-lg font-bold tracking-wider ${
                          rankInfo ? 'text-red-900' : 'text-yellow-100'
                        }`}>
                          {winner.luckyNumber}
                        </span>
                        <span className={`text-[13px] font-semibold ${
                          rankInfo ? 'text-red-900/80' : 'text-yellow-500'
                        }`}>
                          {winner.fullName}
                        </span>
                        ...
                        <span className={`text-[12px] font-semibold ${
                          rankInfo ? 'text-red-900/80' : 'text-yellow-600'
                        }`}>
